// A game is just a sequence of game states on a given field. Each
// state has the position of all the players, discs and any other on
// field objects that there may be; a state also has an associated
// description string.
// As fucking simple as that!
var Game = can.Model({}, {});

// A Model to model the state of the field at a key-frame.
var State = can.Model({}, {
  ...
  init: function(players) {
    ...
    this.players = new can.List(players);
    this.players.bind('change', function(ev, index, type, val) {
...
var Player = can.Model({
  ...
  init: function(team, x, y){
    ...
    Player.count += 1;
    this.attr('team', team || 'o');
    this.attr('x', x || 0);
    this.attr('y', y || 0);

    // Notify when x or y changes.
    this.bind('change', function(ev, property, val){
      console.log('x position changed for player:', ev, property, val);
    });

  },

  move_player: function(new_x, new_y) {
    this.attr('x', new_x);
    this.attr('y', new_y);
  }
...
var GameView = can.Control({},{

  init: function(el, options){
    console.log(el);
    // create State objects from options
    // Initialize Game.
    var game = Game();
    console.log(game);
  }
...
// var canvas = Raphael($('.layout')[0], 1000, 1000);

// var game_view = new GameView(canvas, {});
